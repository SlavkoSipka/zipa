...
import CategoryForm from '../../components/forms/faqCategoryForm';
...
class CategoryPage extends Component {
    ...
    componentDidMount() {
        ...
        fetch(`${API_ENDPOINT}/faqCategories/get/` + this.props[0].match.params.id, {
            method: 'GET',
            headers: {
    ...
    submit(data) {
        console.log(data);
        fetch(`${API_ENDPOINT}/faqCategories/update/` + this.props[0].match.params.id, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${localStorage.getItem('authToken')}`
            ...
            if (result.error) {
                ...
                this.props[0].history.push('/account/faqCategories')
            }
        })
    ...
    render() {
        return (
            <AdminOkvir
                ...
                naslov={'FAQ kategorija'.translate(this.props.lang)}
                radnja={<Link to="/account/faqCategories" className="z-adminokvir__radnja z-adminokvir__radnja--tiha">{'Nazad na spisak'.translate(this.props.lang)}</Link>}
            >
