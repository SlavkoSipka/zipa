...
import CategoryForm from '../../components/forms/bannerForm';
...
class CategoryPage extends Component {
    ...
    componentDidMount() {
        ...
        fetch(`${API_ENDPOINT}/banners/get/` + this.props[0].match.params.id, {
            method: 'GET',
            headers: {
    ...
    submit(data) {
        console.log(data);
        fetch(`${API_ENDPOINT}/banners/update/` + this.props[0].match.params.id, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${localStorage.getItem('authToken')}`
            ...
            if (result.error) {
                ...
                this.props[0].history.push('/account/banners')
            }
        })
    ...
    render() {
        return (
            <AdminOkvir
                ...
                naslov={'Baner'.translate(this.props.lang)}
                radnja={<Link to="/account/banners" className="z-adminokvir__radnja z-adminokvir__radnja--tiha">{'Nazad na spisak'.translate(this.props.lang)}</Link>}
            >
